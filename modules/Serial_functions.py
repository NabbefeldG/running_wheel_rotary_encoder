...
def search_for_teensy_module(name, baudrate=9600):
    ports_found = list_ports.comports()
    for port in ports_found:
        try:
            serial_obj = open_serial(port.device, baudrate=baudrate)
            sleep(0.1)  # Had issues on some PCs that were solved by adding this delay
